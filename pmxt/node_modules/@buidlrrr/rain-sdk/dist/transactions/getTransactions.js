...
import { ENTITY_FIELDS, ENTITY_CONFIG, TAKER_ENTITIES, getEnabledEntities, parseEntity, } from './shared.js';
function buildWhereClause(addressField, address, marketAddress, fromTimestamp, toTimestamp) {
    const parts = [`${addressField}: "${address}"`];
    if (marketAddress)
        parts.push(`poolAddress: "${marketAddress}"`);
    if (fromTimestamp !== undefined)
        parts.push(`blockTimestamp_gte: "${fromTimestamp.toString()}"`);
    if (toTimestamp !== undefined)
        parts.push(`blockTimestamp_lte: "${toTimestamp.toString()}"`);
    return `{ ${parts.join(', ')} }`;
}
function buildQuery(params) {
    ...
    for (const entity of enabledEntities) {
        ...
        parts.push(`  ${entity}(where: ${where}, orderBy: blockTimestamp, orderDirection: ${orderDirection}, first: ${limit}) { ${fields} }`);
        ...
        if (entity in TAKER_ENTITIES) {
            ...
            parts.push(`  ${entity}AsTaker: ${entity}(where: ${takerWhere}, orderBy: blockTimestamp, orderDirection: ${orderDirection}, first: ${limit}) { ${fields} }`);
        }
    }
    return `{\n${parts.join('\n')}\n}`;
}
export async function getTransactions(params) {
    const { address, marketAddress, types, fromTimestamp, toTimestamp, first = 20, skip = 0, orderDirection = 'desc', subgraphUrl, subgraphApiKey, } = params;
