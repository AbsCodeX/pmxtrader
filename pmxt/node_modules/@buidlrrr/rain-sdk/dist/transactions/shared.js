...
export function parseEntity(entity, raw) {
    ...
    const base = {
        type: cfg.type,
        id: raw.id,
        marketAddress: raw.poolAddress,
        transactionHash: raw.transactionHash,
        blockNumber: BigInt(raw.blockNumber),
        timestamp: BigInt(raw.blockTimestamp),
        wallet: (raw.wallet ?? raw.maker ?? raw.orderCreator ?? raw.taker),
    };
    // Trading fields
    if (raw.option !== undefined)
        base.option = Number(raw.option);
    if (raw.orderOption !== undefined)
        base.option = Number(raw.orderOption);
    if (raw.baseAmount !== undefined)
        base.baseAmount = BigInt(raw.baseAmount);
    if (raw.optionAmount !== undefined)
        base.optionAmount = BigInt(raw.optionAmount);
    if (raw.orderAmount !== undefined)
        base.optionAmount = BigInt(raw.orderAmount);
    if (raw.orderPrice !== undefined)
        base.price = BigInt(raw.orderPrice);
    if (raw.orderID !== undefined)
        base.orderId = Number(raw.orderID);
    // Maker/taker
    if (raw.maker)
        base.maker = raw.maker;
    if (raw.taker)
        base.taker = raw.taker;
    // Claim fields
    if (raw.winnerOption !== undefined)
        base.winnerOption = Number(raw.winnerOption);
    if (raw.reward !== undefined)
        base.reward = BigInt(raw.reward);
    if (raw.liquidityReward !== undefined)
        base.liquidityReward = BigInt(raw.liquidityReward);
    if (raw.totalReward !== undefined)
        base.totalReward = BigInt(raw.totalReward);
    return base;
}
