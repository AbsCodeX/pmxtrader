...
function buildWhereClause(poolAddress, optionIndex, from, to) {
    const parts = [
        `poolAddress: "${poolAddress}"`,
    ];
    if (from !== undefined)
        parts.push(`blockTimestamp_gte: "${from.toString()}"`);
    if (to !== undefined)
        parts.push(`blockTimestamp_lte: "${to.toString()}"`);
    return `{ ${parts.join(', ')} }`;
}
function buildOptionWhereClause(poolAddress, optionField, optionIndex, from, to) {
    const parts = [
        `poolAddress: "${poolAddress}"`,
...
function buildQuery(poolAddress, optionIndex, from, to) {
...
function extractPricePoints(data, optionIndex) {
    const points = [];
...
function aggregateCandles(points, interval, limit) {
    if (points.length === 0)
        return [];
...
export async function getPriceHistory(params) {
    const { marketId, optionIndex, interval, from, to, limit = 200, subgraphUrl, subgraphApiKey, apiUrl, } = params;
    if (!marketId)
        throw new Error('marketId is required');
    if (!apiUrl)
        throw new Error('apiUrl is required');
    if (!subgraphUrl)
        throw new Error('subgraphUrl is required');
    if (optionIndex < 0)
        throw new Error('optionIndex must be >= 0');
    ...
    const contractAddress = apiData.contractAddress;
    if (!contractAddress) {
        throw new Error('Market response missing contractAddress');
    }
    const apiOptions = apiData.options ?? apiData.choices ?? [];
    if (optionIndex >= apiOptions.length) {
        throw new Error(`optionIndex ${optionIndex} out of range — market has ${apiOptions.length} options`);
    }
    // 2. Query subgraph for trade events
    const query = buildQuery(contractAddress, optionIndex, from, to);
    const data = await subgraphQuery(subgraphUrl, query, subgraphApiKey);
