...
export async function getLeaderboard(params) {
    const { timeframe, limit = 100, subgraphUrl, subgraphApiKey, } = params;
    const now = Math.floor(Date.now() / 1000);
    const fromTimestamp = timeframe === 'all-time' ? 0 : now - TIMEFRAME_SECONDS[timeframe];
    const wallets = new Map();
    function addVolume(address, amount, poolAddress) {
    ...
    for (const { entity, walletFields, amountField, poolField } of entityQueries) {
        let cursor = '';
        while (true) {
            const whereClause = fromTimestamp > 0
                ? cursor
    ...
    const sorted = Array.from(wallets.entries())
        ...
        .slice(0, limit);
    const entries = sorted.map(([addr, agg], i) => ({
        rank: i + 1,
        address: addr,
